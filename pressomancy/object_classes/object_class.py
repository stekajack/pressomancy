...
class EspressoPart(metaclass=Simulation_Object):
    '''
    Class that contains "normal" espresso particle relevant paramaters and methods. At construction one must pass an espresso handle because the class manages parameters that are both internal and external to espresso. There can be multiple instances of espresso particles. Therefore all relevant parameters are instance specific.

    This serves as an interface to incorporate the handeling of normal espresso particles, as one would usualy do with system.part. This way, one can seemelesly incorporate espresso custom particles into the pressomancy framework.
    '''
    required_features=list()	
    numInstances = 0
    ...
    def __init__(self, config: ObjectConfigParams):
        '''
        Initialisation of an espresso particle object requires the specification of particle size, number of parts and a handle to the espresso system
        '''
        EspressoPart.numInstances+= 1
        self.sys=config['espresso_handle']
        if 'type' not in config:
            assert 'type_name' in config, "you must input 'type_name' (real, virtual, ...)"
            assert config['type_name'] in self.part_types, "to create a new type you must add 'type_name' and 'type' "
            config['type'] = self.part_types[config['type_name']]
        self.params=config
        self.associated_objects=self.params['associated_objects']
        self.who_am_i = EspressoPart.numInstances
    ...
    def set_object(self,  pos, ori, **kwargs):
        ...
        :kwargs: np.array() | any other individual particle characteristic (like dipole moment or orientation)
        :return: None
